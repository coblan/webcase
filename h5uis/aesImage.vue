<template>
    <img @load="onload" :src="real_src" alt="">
</template>
<script>
export default{
    props:{
    ...
    methods:{
        ...
        async adaptSrc(){
          ...
          if( search.pathname.endsWith('.aes')){
            ...
            // this.src
            //   var rt = await ex.getFile('/media/general_upload/202209/2022_09/66c2b1e9c82d0f4dd8475d216c27c0c7.png')
            var rt = await ex.getFile(this.src)
            ...
            var base = await Decrypt(rt,aes_info.key)
            // var base =  await blobToBase64(de)
